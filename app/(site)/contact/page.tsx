...
const categories = [
  "Startup", "Small Business", "Agency", "Enterprise", "E-commerce",
  "SaaS", "Non-Profit", "Other",
];
    ...
    <div className="grid gap-[1px] overflow-hidden rounded-[24px] bg-black/30 backdrop-blur-xl lg:grid-cols-[1fr_260px]">
      ...
      <div className="bg-transparent p-5 sm:p-8">
        {submitted ? (
          <div className="flex min-h-[320px] flex-col items-center justify-center gap-4 text-center">
            <div className="flex h-16 w-16 items-center justify-center rounded-full bg-white/10 text-3xl text-foreground">✓</div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid gap-3 sm:grid-cols-2">
              <div>
                ...
                <input name="firstName" required placeholder="Adam" value={form.firstName} onChange={handleChange} className={inputCls} />
              </div>
              <div>
                <label className="mb-1.5 block text-xs text-foreground">Last Name*</label>
                <input name="lastName" required placeholder="Custo" value={form.lastName} onChange={handleChange} className={inputCls} />
              </div>
            </div>

            <div>
              <label className="mb-1.5 block text-xs text-foreground">How can we reach you?*</label>
              <input name="email" type="email" required placeholder="adam@framer.com" value={form.email} onChange={handleChange} className={inputCls} />
            </div>

            <div className="grid gap-3 sm:grid-cols-2">
              <div className="relative">
                <label className="mb-1.5 block text-xs text-foreground">Where Are you from?*</label>
                <select name="country" required value={form.country} onChange={handleChange} className={selectCls}>
                  <option value="" disabled>Select your country...</option>
                  {countries.map((c) => <option key={c} value={c} className="bg-[#111] text-foreground">{c}</option>)}
                </select>
                <span className="pointer-events-none absolute bottom-3.5 right-4 text-muted">▾</span>
              </div>
              <div className="relative">
                <label className="mb-1.5 block text-xs text-foreground">What&apos;s the type of your company?*</label>
                <select name="category" required value={form.category} onChange={handleChange} className={selectCls}>
                  <option value="" disabled>Select Category</option>
                  {categories.map((c) => <option key={c} value={c} className="bg-[#111] text-foreground">{c}</option>)}
                </select>
                <span className="pointer-events-none absolute bottom-3.5 right-4 text-muted">▾</span>
              </div>
            </div>
            ...
            <div>
              ...
              <textarea name="message" required rows={6} placeholder="Type your message..." value={form.message} onChange={handleChange} className="w-full rounded-[18px] bg-white/[0.06] px-5 py-3.5 text-sm text-foreground placeholder:text-muted outline-none focus:bg-white/[0.09] transition-colors resize-none" />
            </div>

            <button type="submit" className="w-full rounded-full bg-white py-3.5 text-sm font-semibold text-black transition hover:bg-white/90">
              Submit Now
            </button>
          </form>
        )}
      ...
      <div className="flex flex-col gap-3 bg-transparent p-4">
        <div className="rounded-[18px] bg-white/[0.04] p-5">
          ...
          <a href="mailto:hello@bigtopsocial.media" className="mt-3 block text-sm font-medium text-foreground transition hover:text-foreground/80">
            hello@bigtopsocial.media
          </a>
        </div>
        <div className="rounded-[18px] bg-white/[0.04] p-5">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-muted">Phone</p>
          <a href="tel:+40003454567" className="mt-3 block text-sm font-medium text-foreground transition hover:text-foreground/80">
            4(000) 345-4567
          </a>
        </div>
        <div className="rounded-[18px] bg-white/[0.04] p-5">
    ...
    <>
      ...
      <section className="relative min-h-[100svh] overflow-visible pb-16 pt-28 lg:hidden">
        <div className="pointer-events-none absolute inset-0 overflow-hidden">
          <video className="absolute inset-0 h-full w-full object-cover object-center" autoPlay muted loop playsInline preload="metadata" aria-hidden>
        ...
        <Container className="relative z-10">
          <Reveal className="flex flex-col items-center text-center">
            ...
            <p className="mt-4 max-w-sm text-sm leading-relaxed text-muted">
              Whether you need clarity, strategy, or full-scale execution — our team
